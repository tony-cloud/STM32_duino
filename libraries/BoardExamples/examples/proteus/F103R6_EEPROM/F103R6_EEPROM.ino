...
void setup() {
  ...
  Serial.begin(115200);
  delay(1000);
  Serial << "Press any key from Serial to begin ......\r\n";
  ...
  if (EEPROM.begin())
    ...
    Serial << "eeprom init ok!  at device adr 0x" << _HEX(EEPROM.pdata->adr) << " is AT24C";
  ...
  for (uint16_t j = 0; j < 16; j++) {
    Serial.println(EEPROM.read(j),HEX);  // read(addr) func
  }
}
